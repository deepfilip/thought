                        ...
                        const std::vector<unsigned char>& vchPlaintext = std::vector<unsigned char>())
{
    ...
    // Output buffers after a failed padded CBC decrypt are not part of the
    // contract and differ across OpenSSL implementations. Compare plaintext
    // only when both implementations report success.
    if (result1 && result2) {
